  ...
  react: function(event, data) {
    switch (event) {
      case 'home': {
        ...
        var greetings = [
          'Погнали! 🚀', 'За работу! 💪', 'Я соскучился! 🐼',
          'Давай решать! 🧮', 'Начинаем! ✨', 'Ты готов? 🤩',
          'Снова вместе! 🌈', 'Жду не дождусь! ⏳', 'Покажи, на что способен! 🔥',
          'Математика ждёт! 📐', 'Погнали решать! 🎯', 'Новый день — новые задачи! 🌟',
          'Вперёд! ⚡', 'Ты справишься! 💪', 'Давай, я с тобой! 🤝'
        ];
        var nudgeMsgs = [
          'Не теряй стрик! Занимайся сегодня 🔥', 'Сегодня без урока? Давай наверстаем! 📚',
          'Твой стрик под угрозой! 🛑', 'Пропустишь день — потеряешь стрик! ⚠️',
          'Давай хоть одну задачку! 🧩', 'Не ленись, зайди поиграть! 🐼',
          'Стрик сам себя не сделает! 🔥', 'Один день — и всё насмарку! 😱',
          'Ты же не хочешь начинать заново? 🔄', 'Занимайся сегодня, не откладывай! ⏰'
        ];
        var startMsgs = [
          'Давай начнём стрик! 🐼', 'Первый шаг — самый важный! 🌟',
          'Начни сегодня! 🔥', 'Сделай первый шаг! 💪',
          'Пришло время начать! ⏳', 'Жду твой первый правильный ответ! ✨',
          'Всё когда-то начинается с первого раза! 🚀', 'Давай зажжём стрик! 🔥',
          'Начни свой путь математика! 📐', 'Первый шаг уже ждёт тебя! 🎯'
        ];
        var streakMsgs = [
          '🔥 Стрик ' + streak + ' дней! Молодец!', 'Уже ' + streak + ' дней! Ты машина! 🤖',
          streak + ' дней подряд! Круто! 🎉', 'Не останавливайся! ' + streak + ' дней! ⚡',
          streak + '-дневный стрик! Ты лучший! 🏆', 'Огонь! ' + streak + ' дней! 🔥',
          'Стрик ' + streak + '! Так держать! 💪', streak + ' дней — новый рекорд! 📈',
          'Ты в ударе! ' + streak + ' дней! 🌟', 'Железный характер! ' + streak + ' дней! 🧠'
        ];
        if (lastPlayed !== today) {
          this.show('nudge', streak > 0 ? nudgeMsgs[Math.floor(Math.random() * nudgeMsgs.length)] : startMsgs[Math.floor(Math.random() * startMsgs.length)], 5000);
        } else if (streak >= 7) {
          this.show('celebrate', streakMsgs[Math.floor(Math.random() * streakMsgs.length)], 4000);
        } else {
          this.show('greeting', greetings[Math.floor(Math.random() * greetings.length)], 4000);
        }
        break;
      }
      case 'correct': {
      ...
      case 'streak': {
        var streakMsgsEvent = [
          '🔥 Стрик ' + data + ' дней!', data + ' дней подряд! Круто! 🎉',
          'Уже ' + data + ' дней! Не останавливайся! ⚡', 'Твой стрик: ' + data + '! Огонь! 🔥',
          data + ' дней! Ты суперзвезда! 🌟', 'Невероятно! ' + data + ' дней! 🏆',
          'Жги! ' + data + ' дней! 💪', data + ' дней — новый рекорд! 📈',
          'Стрик ' + data + ' дней! Ты лучший! 🤩', 'Железная дисциплина! ' + data + ' дней! 🧠'
        ];
        this.show('celebrate', streakMsgsEvent[Math.floor(Math.random() * streakMsgsEvent.length)], 3000);
        break;
      }
      case 'levelup': {
        var lvlMsgs = [
          '🎉 Новый уровень!', 'Поздравляю с новым уровнем! 🥳',
          'Уровень повышен! Ты растешь! 📈', 'Новый уровень — новый ты! ⚡',
          'Ты становишься сильнее! 💪', 'Уровень +1! Круто! 🌟',
          'Прокачка удалась! 🎮', 'Ещё один уровень позади! 🏆',
          'Ты набираешь обороты! 🚀', 'Уровень вырос! Так держать! 🔥',
          'Новый уровень! Горжусь тобой! 🐼', 'Вверх по лестнице успеха! 🪜',
          'Твой уровень растёт! ✨', 'Уровень апнули! Класс! 🎯',
          'С каждым уровнем ты всё круче! 💫'
        ];
        this.show('celebrate', lvlMsgs[Math.floor(Math.random() * lvlMsgs.length)], 3000);
        break;
      }
      case 'complete': {
        var completeMsgs = [
          'Уровень пройден! 🏆', 'Ещё один уровень готов! ✅',
          'Ты справился! Молодец! 🌟', 'Уровень сдан! Отлично! 🎉',
          'Победа! Так держать! ⚡', 'Задание выполнено! 🔥',
          'Уровень позади! Ты крут! 💪', 'Справился на отлично! 💫',
          'Есть! Уровень пройден! 🎯', 'Ты уничтожил этот уровень! 🔥',
          'Готово! Идём дальше! 🚀', 'Уровень в кармане! 🧠',
          'Отличная работа! 🌈', 'Ты закрыл уровень! 🎮',
          'Прогресс есть! Так держать! 📈'
        ];
        this.show('celebrate', completeMsgs[Math.floor(Math.random() * completeMsgs.length)], 3000);
        break;
      }
      case 'boss_defeat': {
        var bossMsgs = [
          'Босс побеждён! 👑', 'Ты одолел босса! 🎉',
          'Босс повержен! Ты герой! ⚔️', 'Никто не устоит перед тобой! 🔥',
          'Босс уничтожен! Поздравляю! 🏆', 'Ты сильнее любого босса! 💪',
          'Босс пал! Слава победителю! 🌟', 'Ещё один босс повержен! 🎮',
          'Ты — чемпион! Босс побеждён! 🥇', 'Босс не шанс! Ты лучший! 🔥',
          'С боссом покончено! Молодец! ✅', 'Босс отправлен в нокаут! 💥',
          'Ты непобедим! Босс сдался! 🏆', 'Босс побеждён! Твой уровень — Бог! ⚡',
          'Легендарная победа! 👊'
        ];
        this.show('celebrate', bossMsgs[Math.floor(Math.random() * bossMsgs.length)], 3000);
        break;
      }
    }
  }
